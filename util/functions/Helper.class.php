...
class Helper {
    public static $DIRECTORIO_PRINCIPAL = "farmacia";
            ...
            $html_mensaje = '
                    <html>
                        <head>
                            ...
                            <link rel="stylesheet" type="text/css" href="../util/tema/style.css" />
    ...
    public function correlativo($tabla) {
        try {
            $sql = "SELECT (numero + 1) as num from correlativo where tabla = :p_tabla";
            $sentencia = $this->dblink->prepare($sql);
            $sentencia->bindParam(":p_tabla", $tabla);
            $sentencia->execute();
            if ($sentencia->rowCount()) {
                $resultado = $sentencia->fetch(PDO::FETCH_ASSOC);
                return $resultado["num"];
            } else {
                // throw new Exception("No se ha configurado el correlativo para la tabla ".$tabla);
                $sql = "INSERT INTO correlativo values (:p_tabla,1);";
                $sentencia = $this->dblink->prepare($sql);
                $sentencia->bindParam(":p_tabla", $tabla);
                $sentencia->execute();
                return 0;
    ...
    public function actualizarCorrelativo($tabla, $numero) {
        try {
            $sql = "UPDATE correlativo set numero = :p_numero where tabla = :p_tabla";
            $sentencia = $this->dblink->prepare($sql);
            $sentencia->bindParam(":p_tabla", $tabla);
            $sentencia->bindParam(":p_numero", $numero);
            $sentencia->execute();
        } catch (PDOException $e) {
            throw new Exception($e->getMessage());
        }
